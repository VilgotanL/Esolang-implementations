...
async function transpileBfWithIfToBf1(code) {
    let newCode = "";
    ...
    for(let i=0; i<code.length; i++) {
        if(code[i] === "+" || code[i] === "-" || code[i] === "." || code[i] === "," || code[i] === "[" || code[i] === "]") {
            ...
            newCode += "<<";
        } else if(code[i] === "(") { //() instructions mean if 0
            newCode += "[->+>>+<<<]>>>[-<<<+>>>]+<<[[-]>>-<<]>>[-<<<";
        } else if(code[i] === ")") {
            newCode += ">>>]<<<";
        } else if(code[i] === "{") { //{} instructions mean if not 0
            newCode += "[->+>>+<<<]>>>[-<<<+>>>]<<[[-]<";
        } else if(code[i] === "}") {
            newCode += ">]<";
        }
...
transpileBtn.addEventListener("click", async function() {
    ...
    let transpiled = await transpileBfWithIfToBf1(code);
    output(transpiled);
